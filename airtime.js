...
function switchMode(mode) {
    ...
    // Show/hide network selection section (only visible in single mode)
    document.getElementById('networkSection').style.display = mode === 'single' ? 'block' : 'none';
    ...
    document.getElementById('summaryAmountRow').style.display = mode === 'single' ? 'flex' : 'none';
    document.getElementById('summaryRecipientsBreakdown').style.display = mode === 'bulk' ? 'block' : 'none';
...
function addRecipient() {
    ...
    const amountInput = recipientItem.querySelector('.recipient-amount-input');
    amountInput.addEventListener('input', function() {
        // Clear error on input
        const error = recipientItem.querySelector('.recipient-amount-error');
        error.classList.remove('show');
        this.classList.remove('error');
...
function updateBulkSummary() {
    const recipientItems = document.querySelectorAll('.recipient-item');
    const breakdownDiv = document.getElementById('summaryRecipientsBreakdown');
    
    // Collect valid recipients with their amounts
    ...
    recipientItems.forEach((item, index) => {
        ...
        const amount = parseFloat(item.querySelector('.recipient-amount-input').value) || 0;
        
        if (phone !== '' && selectedNetworkBtn && validatePhone(phone) && amount > 0) {
    ...
    // Show recipients breakdown
    if (validRecipients.length > 0) {
        breakdownDiv.style.display = 'block';
        breakdownDiv.innerHTML = '<div style="margin: 10px 0; padding-top: 10px; border-top: 1px solid rgba(255,255,255,0.1);">';
        breakdownDiv.innerHTML += '<div style="margin-bottom: 8px; color: #4CAF50; font-weight: 600; font-size: 13px;">Recipients:</div>';
...
function validateBulkPurchase() {
    ...
    recipientItems.forEach((item, index) => {
        ...
        const selectedNetworkBtn = item.querySelector('.recipient-network-btn.selected');
        const amountInput = item.querySelector('.recipient-amount-input');
        const amountError = item.querySelector('.recipient-amount-error');
        const amount = parseFloat(amountInput.value);
        
        // Skip completely empty recipients
        if (phone === '' && !selectedNetworkBtn && !amount) {
            return;
        }
        ...
        // Validate amount
        if (!amount || amount < MIN_AMOUNT) {
            missingAmountRecipients.push(index + 1);
            amountError.textContent = `Amount must be at least ₵${MIN_AMOUNT}`;
            amountError.classList.add('show');
            amountInput.classList.add('error');
            isValid = false;
    ...
    if (missingAmountRecipients.length > 0) {
        showAlert(`Please enter amount for recipient ${missingAmountRecipients.join(', ')}`, 'error');
        return false;
    }
...
function showBulkSuccess() {
    ...
    // Add notification
    addNotification('success', 'Bulk Purchase Successful', `Airtime sent to ${validRecipients.length} recipients (Total: ₵${totalAmount.toFixed(2)})`);
...
function resetForm() {
    ...
    document.getElementById('summaryRecipientsBreakdown').style.display = 'none';
